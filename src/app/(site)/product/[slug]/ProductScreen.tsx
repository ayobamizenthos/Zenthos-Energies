...
export default function ProductPage({ initialProduct }: { initialProduct?: Product }) {
  ...
  const related = (() => {
    const list = sameCategory.filter(p => p.id !== product.id)
    const cable = cables.find(c => c.id !== product.id)
    const panel = panels.find(p => p.id !== product.id)
    if (panel && !list.some(p => p.id === panel.id)) list.push(panel)
    if (cable && !list.some(p => p.id === cable.id)) list.push(cable)
    return list
  })()
